...
const DndWrapper: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const handleDragOver = useCallback(
    ({ active, over }: DragOverEvent) => {
      ...
      if (!activePage && active.data.current?.dndType === "element") {
        ...
        newIndex =
          overIndex >= 0 ? overIndex + modifier : overFields.length + 1;
  ...
  const onDragEnd = useCallback(
    ({ active, over }: DragEndEvent) => {
      ...
      if (overPage) {
        ...
        moveField(uniqueId, overPage, activeIndex, overIndex);
      }
      setActiveElement(null);
    },
    [formMetaData, activeElement]
  ...
  const onDragCancel = useCallback(() => {
    if (clonedMetaData && activeElement.data.current?.dndType === "element") {
